...
var ZSDKMessageManager = (function(ZSDKMessageManager) {
  ...
  function isAuthorizedMessage(MEvent) {
    if (!MEvent || !SDKContext || typeof SDKContext.isAppRegistered !== 'function') {
      return false;
    }

    if (!SDKContext.isAppRegistered() || !AuthParentWindow || !AuthParentOrigin) {
      return false;
    }

    return MEvent.source === AuthParentWindow && MEvent.origin === AuthParentOrigin;
  }
  function isTrustedSetContextMessage(MessageEvent) {
    return (
      MessageEvent &&
      MessageEvent.source === window.parent &&
      MessageEvent.origin === SDKContext.QueryParams.serviceOrigin
    );
  }
  function MessageHandler(MessageEvent) {
    ...
    try {
      var isTrustedMessage = eventName === 'SET_CONTEXT' ? isTrustedSetContextMessage(MessageEvent) : isAuthorizedMessage(MessageEvent);

      if (!isTrustedMessage) {
        Logger.Error('Un-Authorized Message.', MessageEvent && MessageEvent.origin);
        return;
      }
  ...
  function HandleSetContext(MessageEvent, payload) {
    ...
    AuthParentOrigin = MessageEvent.origin;
